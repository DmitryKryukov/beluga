...
export default {
  ...
  mounted() {
    ...
    this.pager.page.currentApprox = 1;
  },
  watch: {
    "pager.page.current": function(newVal, oldVal) {
      ...
      this.$refs.pager.style.maxHeight = pageHeight + "px";
  ...
  methods: {
    init() {
      ...
      this.pager.innerWidth = this.$refs.pager.scrollWidth;
      this.pager.page.width = window.innerWidth;
      this.pager.page.count = this.pager.innerWidth / this.pager.page.width;
      this.pager.page.current = 1;
      //Получаем все объекты в скроллере и сохраняем их ширину и позицию
      let tabLinks = {
      ...
      tabLinks.el.forEach(function(tabLink) {
        tabLinks.lefts.push(tabLink.offsetLeft + indicatorMargin);
        tabLinks.widths.push(tabLink.offsetWidth - indicatorMargin * 2);
      });
      this.pager.top = this.$refs.pager.offsetTop - 20;
      //this.$refs.pager.style.minHeight = "calc(100vh - " + this.pager.top + "px)";
